...
public:
    Test(ExtraChainNode *node, QObject *p = nullptr)
        ...
        Transaction tr1(node->getAccountController()->getCurrentActor().id(), BigNumber("322323dddaa"),
                        BigNumber(23));
        Transaction tr2(node->getAccountController()->getCurrentActor().id(), BigNumber("234234aaaa"),
                        BigNumber(45));
        Transaction tr3(node->getAccountController()->getCurrentActor().id(), BigNumber("23aaaaaaaaaa"),
                        BigNumber(4));
